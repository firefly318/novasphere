...
export default function GoodsReceipt() {
  ...
  const handleAddItemRow = () => {
    setItems([...items, { MaterialId: '', SoLuong: 1, DonGiaNhap: 0 }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate
    const invalidItem = items.some(item => !item.MaterialId || item.SoLuong <= 0 || item.DonGiaNhap < 0);
    if (invalidItem) {
      setSubmitError('Vui lòng chọn vật tư và nhập số lượng/đơn giá lớn hơn 0.');
      return;
    }
    ...
    <div className="bg-white border border-sky-100 p-6 rounded-2xl shadow-sm relative">
      ...
      {activeTab === 'create' && (
        <form onSubmit={handleSubmit} className="space-y-6 no-print">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">NHÀ CUNG CẤP *</label>
            <select
              required
              value={supplierId}
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-sky-100 pb-2">
              <h3 className="text-sm font-semibold text-slate-700">Danh sách vật tư nhập kho</h3>
              <button
                type="button"
                ...
                <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-slate-50/50 p-3 border border-sky-100 rounded-xl">
                  ...
                  <div className="md:col-span-6">
                    ...
                    <MaterialSelect
                      ...
                      materials={materials}
                      placeholder="-- Chọn vật tư cần nhập --"
                    />
                  </div>

                  {/* Số lượng */}
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-semibold text-slate-400 mb-1">SỐ LƯỢNG *</label>
                    <input
                      type="number"
                      required
                      min="1"
                  ...
                  <div className="md:col-span-3">
                    <label className="block text-[10px] font-semibold text-slate-400 mb-1">ĐƠN GIÁ NHẬP (VNĐ) *</label>
                    <input
                      type="number"
                      required
                      min="0"
          ...
          <div className="bg-white border border-sky-100 rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] no-print text-slate-800">
            ...
            <div className="p-6 space-y-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-2 gap-4 bg-sky-50/20 p-4 border border-sky-100 rounded-xl">
                ...
                <div>
                  <p className="text-[10px] font-semibold text-slate-455">NHÀ CUNG CẤP</p>
                  <p className="text-xs text-slate-600 mt-0.5 truncate">{selectedReceipt.GhiChu}</p>
                </div>
              </div>
      ...
      {selectedReceipt && (
        <div className="hidden print:block print-container font-sans text-black p-8">
          ...
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold uppercase">Phiếu Nhập Kho Vật Tư</h2>
          ...
          <div className="mb-6 grid grid-cols-2 gap-4 text-sm leading-relaxed">
            <div>
              ...
              <p><strong>Nhà cung cấp:</strong> {selectedReceipt.GhiChu}</p>
            </div>
